	...
	for (l = 0; l <= length2 - 1; l++)	{
		for (i = 1; i <= length2 - l; i++)	{
				...
				if (tlist1[j].flag == 1)	{
					...
					//1
					if (l != 0) {
						temp_a = (int)(getAlignVal(tl,i+1,i+l-1) +/* weight[seq2[i]][seq1[tp1]] 
                                                        + weight[seq2[i+l]][seq1[tp2]] */
                                                        + p_w[seq2[i]][seq2[i+l]][seq1[tp1]][seq1[tp2]]
/*dan*/                                                 * stkpf->p_profile[seq2[i] * 4 + seq2[i+l]][tp1-1]);
					...
					//5
					temp_b = (int)(getAlignVal(tl,i+1,i+l) + weight[seq2[i]][seq1[tp1]]
/*dan*/                                  * stkpf->profile[seq2[i]][tp1-1]
                                                  + g2);
					if (temp_b > temp_a)	{
						temp_a = temp_b;
						temp_t = 5;
...
/*dan*/                                  * stkpf->profile[seq2[i+l]][tp2-1]
                                                 + g2);
					if (temp_b > temp_a)	{
						temp_a =  temp_b;
						temp_t = 6;
